...
%DIF ADD article/article_r1.tex   Fri Jul 18 15:40:39 2025
...
\begin{enumerate}
	...
	We predicted that a multi-species model will be able to produce better fitting estimates of cue rate \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend EDR than a single species model, particularly for data sparse species.
...
\par \DIFdelbegin \DIFdel{For any point count that employed removal sampling \mbox{%DIFAUXCMD
...
That is , if we have }\DIFdelend \DIFaddbegin \DIFadd{We will first begin by describing the point count data that are to be used in the models.
...
If }\DIFaddend a point count \DIFdelbegin \DIFdel{where removal sampling or distance sampling (but not both) was not used, then either $J = 1$ or $K = 1$, respectively; whereas, if we have }\DIFdelend \DIFaddbegin \DIFadd{used removal sampling \mbox{%DIFAUXCMD
\citep{alldredge_time--detection_2007, farnsworth_removal_2002}}\hskip0pt%DIFAUXCMD
...
Further, if }\DIFaddend a point count \DIFdelbegin \DIFdel{where both removal sampling and distance sampling was used, then both $J,K \geq 2$.
The case where $J = K = 1$ is where neither removal sampling nor distance sampling was used, and so these data points are not appropriate for the modelling techniques described here}\DIFdelend \DIFaddbegin \DIFadd{used distance sampling \mbox{%DIFAUXCMD
\citep{buckland_introduction_2001, buckland_distance_2015}}\hskip0pt%DIFAUXCMD
...
Thus, we let $Y_{sijk}$ be the observed count of species $s$ during sampling event $i$, occurring in time band $j$ and/or distance band $k$}\DIFaddend .
...
Thus, we consider counts \DIFdelbegin \DIFdel{$Y_{sij.} = \sum_{k=1}^{K}$}\DIFdelend \DIFaddbegin \DIFadd{$Y_{sij.} = \sum_{k=1}^{K}{Y}_{sijk}$}\DIFaddend . 
From \citet{solymos_calibrating_2013}, we model $Y_{sij.}$ as

\DIFdelbegin %DIFDELCMD < \MATHBLOCKDOLLARDOLLAR{%%%
\DIFdel{Y}\DIFdelend \DIFaddbegin $${Y}\DIFaddend _{sij.} \sim \DIFdelbegin \DIFdel{multinomial}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{multinomial}}\DIFaddend \left(\DIFaddbegin {\DIFaddend Y\DIFaddbegin }\DIFaddend _{si..}, \mathbf{\Pi}\DIFdelbegin \DIFdel{_{si.}}\DIFdelend \DIFaddbegin \DIFadd{_{si}^{\left\{\phi\right\}}}\DIFaddend \right).$$

\par For sample $i$, \DIFdelbegin \DIFdel{$\mathbf{\Pi}_{si.}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\Pi}_{si}^{\left\{\phi\right\}}$ }\DIFaddend is the corresponding probability vector that determines how the total \DIFdelbegin \DIFdel{number }\DIFdelend \DIFaddbegin \DIFadd{individuals }\DIFaddend of species $s$ observed at sample $i$ (i.e., \DIFdelbegin \DIFdel{$Y_{si..}$}\DIFdelend \DIFaddbegin \DIFadd{${Y}_{si..}$}\DIFaddend ) are to be distributed across the $J$ time bands; in essence, it is the mixing parameter.
...
We then have the following calculation of each component \DIFdelbegin \DIFdel{$\pi_{sij}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{sij}^{\left\{\phi\right\}}$ }\DIFaddend of the mixing parameter \DIFdelbegin \DIFdel{$\mathbf{\Pi}_{si.}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\Pi}_{si}^{\left\{\phi\right\}}$}\DIFaddend :

\begin{equation*}
	\pi_{sij}\DIFaddbegin \DIFadd{^{\left\{\phi\right\}} }\DIFaddend = 
	\DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < 		\dfrac{\exp\left\{ -t_{i,j-1}\phi_{s} \right\} - \exp\left\{ -t_{ij}\phi_{s} \right\}}{1 - \exp\left\{ -t_{iJ}\phi_{s} \right\}} & \text{for } j > 1 \\
%DIFDELCMD < 		1 - \sum_{n = 2}^{J} \pi_{sin} & \text{for } j = 1
%DIFDELCMD < 	\end{cases}%%%
...
Thus, we consider counts \DIFdelbegin \DIFdel{$Y_{si.k} = \sum_{j=1}^{J}$}\DIFdelend \DIFaddbegin \DIFadd{${Y}_{si.k} = \sum_{j=1}^{J}{Y}_{sijk}$}\DIFaddend . 
From \citet{solymos_calibrating_2013}, we model \DIFdelbegin \DIFdel{$Y_{si.k}$ as
}%DIFDELCMD < \MATHBLOCKDOLLARDOLLAR{%%%
\DIFdel{Y}\DIFdelend \DIFaddbegin \DIFadd{${Y}_{si.k}$ as
}$${Y}\DIFaddend _{si.k} \sim multinomial\left(\DIFaddbegin {\DIFaddend Y\DIFaddbegin }\DIFaddend _{si..}, \mathbf{\Pi}\DIFdelbegin \DIFdel{_{si.}}\DIFdelend \DIFaddbegin \DIFadd{_{si}^{\left\{\tau\right\}}}\DIFaddend \right).$$

\par For sample $i$, \DIFdelbegin \DIFdel{$\mathbf{\Pi}_{si.}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\Pi}_{si}^{\left\{\tau\right\}}$ }\DIFaddend is the corresponding probability vector that determines how the total number of \DIFaddbegin \DIFadd{individuals of }\DIFaddend species $s$ observed at sample $i$ (i.e., \DIFdelbegin \DIFdel{$Y_{si..}$}\DIFdelend \DIFaddbegin \DIFadd{${Y}_{si..}$}\DIFaddend ) are to be distributed across the $K$ distance bins; in essence, it is the mixing parameter. 
...
We then have the following calculation of each component \DIFdelbegin \DIFdel{$\pi_{sik}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{sik}^{\left\{\tau\right\}}$ }\DIFaddend of the mixing parameter \DIFdelbegin \DIFdel{$\mathbf{\Pi}_{si.}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\Pi}_{si}^{\left\{\tau\right\}}$}\DIFaddend :

\begin{equation*}
	\pi_{sik}\DIFaddbegin \DIFadd{^{\left\{\tau\right\}} }\DIFaddend = 
	\DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD < 		\dfrac{f(r_{i,k}, \tau_s) - f(r_{i,k-1}, \tau_s)}{f(r_{i,K}, \tau_s)} & \text{for } k > 1 \\
%DIFDELCMD < 		1 - \sum_{n = 2}^{K} \pi_{sin} & \text{for } k = 1
%DIFDELCMD < 	\end{cases}%%%
\DIFdelend \DIFaddbegin \begin{cases}
...
}\DIFdelend $$f(r,\tau) =  1 - \exp\left\{ -\dfrac{r^2}{\exp\left\{\tau^2\right\}} \right\}.$$
...
\DIFadd{We then set the following priors:
...
, but estimated in a Bayesian rather than maximum likelihood framework}\DIFdelend \DIFaddbegin \DIFadd{$\log \phi_s \sim N(0,1)$, and $\log \tau_s \sim N(0,1)$, implying that the values of $phi$ and $tau$ are drawn as independent samples}\DIFaddend .
