...
%DIF ADD article/article_r1.tex   Fri Jul 11 11:35:31 2025
...
\begin{abstract}
		...
		Here, \DIFaddbegin \DIFadd{based on results of a previous study, }\DIFaddend we take advantage of similarities in detection probabilities among phylogenetically-related species and species with similar traits to estimate detection probabilities in seven species of rare North American birds, using hierarchical Bayesian multi-species removal and distance models. 
...
That is, \DIFdelbegin \DIFdel{$\log \phi_s \sim N(0,1) \forall s$, and $\log \tau_s \sim N(0,1) \forall s$}\DIFdelend \DIFaddbegin \DIFadd{$\log \phi_s \sim N(0,1)$, and $\log \tau_s \sim N(0,1)$}\DIFaddend , thus making it equivalent to the Null model in \citet{edwards_point_2023}, but estimated in a Bayesian rather than maximum likelihood framework. 
...
Rufous Hummingbird (\textit{Selasphorus rufus}) was dropped from the distance modelling due to issues with divergent transitions \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\par \DIFaddend To determine which species would be left out in each fold of the removal model, we first split the phylogenetic tree into 10 parts using the \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend cutree" function in base R, which preserves relatedness in the 10 parts of the split tree.
...
\par \DIFaddend To determine which species would be left out in each fold of the distance model, we randomly assigned a number between 1 and 10 (drawn from a uniform distribution) to each species within a migratory \DIFdelbegin \DIFdel{resident }\DIFdelend \DIFaddbegin \DIFadd{status }\DIFaddend $\times$ habitat preference \DIFaddbegin \DIFadd{grouping combination}\DIFaddend , which ensured that there would be a good balance of species left out for each group of species.
