...
%DIF DEL article/article.tex      Fri Jul  4 13:53:25 2025
%DIF ADD article/article_r1.tex   Fri Jul 25 16:22:30 2025
...
\begin{abstract}
		...
		However, probability of detection is often unaccounted for \DIFdelbegin \DIFdel{or unavailable }\DIFdelend on a species level. 
		...
		Here, \DIFaddbegin \DIFadd{based on results of a previous study investigating trait-based relationships of detectability, }\DIFaddend we take advantage of similarities in detection probabilities among phylogenetically-related species and species with similar traits to estimate detection probabilities in seven species of rare North American birds, using hierarchical Bayesian multi-species removal and distance models. 
		...
		However, when no data were available for a species, the multi-species models presented here tended to \DIFdelbegin \DIFdel{be overly conservative in predicting detectability}\DIFdelend \DIFaddbegin \DIFadd{do a poor job at predicting detectability, based on traits and phylogeny alone}\DIFaddend . 
...
\section{Keywords}
\par hierarchical modelling, distance sampling, removal sampling, QPAD, detectability, effective detection radius, cue rate, phylogenetics, \DIFdelbegin \DIFdel{bayesian
}\DIFdelend \DIFaddbegin \DIFadd{Bayesian
}\DIFaddend 
...
To date, the project has estimated the two components of \DIFdelbegin \DIFdel{detectability--availability }\DIFdelend \DIFaddbegin \DIFadd{detectability---availability }\DIFaddend (the probability that a bird sings or gives a cue during the survey period) and perceptibility (the probability that an observer sees or hears a cueing bird)\DIFdelbegin \DIFdel{--for }\DIFdelend \DIFaddbegin \DIFadd{---for }\DIFaddend 319 species of birds, with an additional 19 species having estimates of either availability or perceptibility. 
...
\DIFaddbegin \subsection{\DIFadd{The QPAD Approach to Estimating Detectability}}
\DIFaddend \par Both the NA-POPS project and previous estimates by Boreal Avian Modelling project use the QPAD approach \citep{solymos_calibrating_2013}, which is a flexible method to estimate availability and perceptibility given a harmonized dataset with the necessary survey-specific data (i.e., distance to bird from observer and time to first detection; \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{barker_ecological_2015}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{barker_ecological_2015}}\hskip0pt%DIFAUXCMD
}\DIFaddend ).
...
\par \DIFaddbegin \DIFadd{Here, we review the QPAD model as derived in \mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
.
...
If a point count used removal sampling \mbox{%DIFAUXCMD
\citep{alldredge_time--detection_2007, farnsworth_removal_2002}}\hskip0pt%DIFAUXCMD
, then these recorded observed abundances will be further subdivided into a time band $j \in [1,J_i]$, where $J_i$ is the maximum number of time bins for sampling event $i$.
Further, if a point count used distance sampling \mbox{%DIFAUXCMD
\citep{buckland_introduction_2001, buckland_distance_2015}}\hskip0pt%DIFAUXCMD
, then these recorded observed abundances will also be further subdivided into a distance bin $k \in [1,K_i]$, where $K_i$ is the maximum number of distance bins for sampling event $i$.
...
\subsubsection{\DIFadd{QPAD Removal Modelling and Availability}}
\par \DIFadd{For removal modelling, we are only interested in the time bin in which the bird was recorded, and not the distance, and so we sum counts from each sampling event $i$ over all distance bands. 
...
$${Y}\DIFadd{_{sij.} \sim \mathrm{multinomial}\left(}{\DIFadd{Y}}\DIFadd{_{si..}, \mathbf{\Pi}_{si}^{\left\{\phi\right\}}\right).}$$

\par \DIFadd{For sample $i$, $\mathbf{\Pi}_{si}^{\left\{\phi\right\}}$ is the corresponding probability vector that determines how the total individuals of species $s$ observed at sample $i$ (i.e., ${Y}_{si..}$) are to be distributed across the $J$ time bands; in essence, it is the mixing parameter.
...
\begin{equation}\DIFadd{\label{eq:removal}
...
\par \DIFadd{We are then interested in estimating $\phi_s$, the cue rate for species $s$.
In \mbox{%DIFAUXCMD
\citep{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
, $\phi_s$ is estimated using maximum likelihood methods, for each species separately, regressed against covariates known to affect cue rate such as time of day and time of year.
Cue rate can then be used to estimate availability, given by
}

\begin{equation}\DIFadd{\label{eq:availability}
	p(t) = 1 - e^{-t\phi},
}\end{equation}
\DIFadd{and defined as the probability that a bird gives a cue within $t$ minutes of a survey.
}

\subsubsection{\DIFadd{QPAD Distance Modelling and Perceptibility}}

\par \DIFadd{For distance modelling, we are only interested in the distance to the cueing bird, and not the time bin that the bird was recorded in, and so we sum counts from each sampling event $i$ over all time bands. 
Thus, we consider counts ${Y}_{si.k} = \sum_{j=1}^{J}{Y}_{sijk}$. 
From \mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
, we model ${Y}_{si.k}$ as
}$${Y}\DIFadd{_{si.k} \sim multinomial\left(}{\DIFadd{Y}}\DIFadd{_{si..}, \mathbf{\Pi}_{si}^{\left\{\tau\right\}}\right).}$$

\par \DIFadd{For sample $i$, $\mathbf{\Pi}_{si}^{\left\{\tau\right\}}$ is the corresponding probability vector that determines how the total number of individuals of species $s$ observed at sample $i$ (i.e., ${Y}_{si..}$) are to be distributed across the $K$ distance bins; in essence, it is the mixing parameter. 
...
\begin{equation}\DIFadd{\label{eq:distance}
...
$$\DIFadd{f(r,\tau) =  1 - \exp\left\{ -\dfrac{r^2}{\exp\left\{\tau^2\right\}} \right\}.}$$

\par \DIFadd{We are most interested in estimating $\tau_s$, the effective detection radius (EDR) for species $s$. 
In \mbox{%DIFAUXCMD
\citep{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
, $\tau_s$ is estimated using maximum likelihood methods, for each species separately, regressed against covariates known to affect EDR such as tree cover.
EDR can then be used to estimate perceptibility, given by
}

\begin{equation}\DIFadd{\label{eq:perceptibility}
	q(r) = \dfrac{\tau^2 \left\{1 - e^{-\dfrac{r^2}{\tau^2}}\right\}}{r^2}
}\end{equation}
\DIFadd{and defined as the probability that a bird is perceived by an observer when the bird is $r$ metres away, given the bird gives a cue.
}

\subsection{\DIFadd{Opportunities to Expand on Estimates of Detectability}}

\par \DIFaddend Despite the efforts of the NA-POPS project, approximately 25\% of species in Partners in Flight’s 2016 Landbird Conservation Plan \citep{rosenberg_partners_2016} have insufficient data to directly estimate detectability \citep{edwards_point_2023}.
...
\citet{johnston_species_2014} showed that detection \DIFdelbegin \DIFdel{distance - and consequently perceptibility - varied }\DIFdelend \DIFaddbegin \DIFadd{distance---and consequently perceptibility---varied }\DIFaddend with body mass and habitat association in a selection of European landbirds.
...
This could be done in two ways: by making use of shrinkage factors and partial pooling, as done in Bayesian hierarchical models \citep{gelman_what_2021, gelman_bayesian_2006}, and by accounting for phylogenetic relationships and the relative \DIFdelbegin \DIFdel{"relatedness " }\DIFdelend \DIFaddbegin \DIFadd{relatedness }\DIFaddend of different species, as \DIFdelbegin \DIFdel{done with }\DIFdelend \DIFaddbegin \DIFadd{can be done with covariance matrices or }\DIFaddend Gaussian processes \citep{bernardo_regression_1998, mcelreath_continous_2020}.
...
To achieve this goal, we \DIFdelbegin \DIFdel{aimed }\DIFdelend \DIFaddbegin \DIFadd{aim }\DIFaddend to satisfy the following objectives:
\begin{enumerate}
	...
	For species with \DIFdelbegin \DIFdel{a sufficient sample size}\DIFdelend \DIFaddbegin \DIFadd{sufficient sample sizes}\DIFaddend , we predicted the mean and precision of cue rate and EDR estimates will be similar from multi- and single-species models \citep{edwards_point_2023}.
	...
	We predicted that a multi-species model will be able to produce better fitting estimates of cue rate \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend EDR than a \DIFdelbegin \DIFdel{single species }\DIFdelend \DIFaddbegin \DIFadd{single-species }\DIFaddend model, particularly for data sparse species.
...
\par We used the same standardized database of point count data that was assembled through the NA-POPS project \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{edwards_point_2023}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{edwards_point_2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
\DIFaddbegin \DIFadd{These species are all on Partners in Flight's watchlist, indicating their conservation importance \mbox{%DIFAUXCMD
\citep{will_handbook_2020}}\hskip0pt%DIFAUXCMD
, and represent a range of different phylogenies and traits within the bird taxon.
We did not include Woodhouse's Scrub-jay (}\textit{\DIFadd{Aphelocoma woodhouseii}}\DIFadd{), Pacific Wren (}\textit{\DIFadd{Troglodytes pacificus}}\DIFadd{), or Sagebrush Sparrow (}\textit{\DIFadd{Artemisiospiza nevadensis}}\DIFadd{) in the analysis, because these species were not available in the phylogenies from \mbox{%DIFAUXCMD
\citet{jetz_global_2012}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend This resulted in 2000 pseudo-posterior trees that contained 323 species.

\par We generated a phylogenetic correlation matrix $\mathbf{C}$ using the same methods outlined in the Appendix of \citet{solymos_phylogeny_2018} \DIFdelbegin \DIFdel{that generate distances between species based on covariance, as processed }\DIFdelend using the R package \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend ape" \citep{paradis_ape_2019}. 
\DIFaddbegin \DIFadd{This correlation matrix is a square matrix that identifies the phylogenetic relatedness of one species to other species.
...
Species were classified as \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend closed habitat" if their primary habitat was forests, and \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend open habitat" if their primary habitat was grasslands or shrublands.
Species with specific areas of their range that are only for breeding (i.e., not year-round throughout their range) were classified as \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend migratory".
...
Body mass for each bird species was obtained from the Elton traits dataset if available \citep{wilman_eltontraits_2014}, and \DIFaddbegin \DIFadd{otherwise }\DIFaddend through the Birds of the World account \citep{billerman_birds_2022} if not.

\subsection{Bayesian Hierarchical Models}

\DIFdelbegin %DIFDELCMD < \par %%%
\DIFdel{For any point count that employed removal sampling \mbox{%DIFAUXCMD
\citep{alldredge_time--detection_2007, farnsworth_removal_2002} }\hskip0pt%DIFAUXCMD
and/or distance sampling \mbox{%DIFAUXCMD
\citep{buckland_introduction_2001, buckland_distance_2015}}\hskip0pt%DIFAUXCMD
, let $Y_{sijk}$ be the observed count of species $s$ during sampling event $i$, occurring in time band $j \in [1,J]$ and/or distance band $k \in [1,K]$.
...
%DIFDELCMD < %%%
\DIFdelend \subsubsection{Bayesian Hierarchical QPAD Removal Model}\DIFaddbegin \label{section-removal-models}
 \DIFaddend 

\par \DIFdelbegin \DIFdel{For removal modelling, we are only interested in the time bin in which the bird was recorded, and not the distance, and so we sum counts from each sampling event $i$ over all distance bands. 
...
%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{Y_{sij.} \sim multinomial\left(Y_{si..}, \mathbf{\Pi}_{si.}\right).}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < \par %%%
\DIFdel{For sample $i$, $\mathbf{\Pi}_{si.}$ is the corresponding probability vector that determines how the total number of species $s$ observed at sample $i$ (i.e., $Y_{si..}$) are to be distributed across the $J$ time bands; in essence, it is the mixing parameter.
Let $t_{ij}$ be the maximum time for time band $j$ during sampling event $i$, and let $\phi_s$ be the unknown cue rate for species $s$.
...
%DIFDELCMD < \par %%%
\DIFdel{We are most interested in estimating $\phi_s$, the cue rate for species $s$}\DIFdelend \DIFaddbegin \DIFadd{Recall from Equation \ref{eq:removal} that the cue rate $\phi$ determines the mixing parameter in a removal model setting}\DIFaddend .
To allow for the partial pooling of \DIFaddbegin \DIFadd{cue rate }\DIFaddend estimates for each species to be based on a phylogenetic relationship, we can \DIFdelbegin \DIFdel{use Gaussian processes, which effectively allow for ``continuous" factors \mbox{%DIFAUXCMD
\citep{bernardo_regression_1998, mcelreath_continous_2020}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{draw estimates of cue rate from a multivariate normal distribution}\DIFaddend . 
Let $\mathbf{\Phi}$ be the vector of cue rates (of length \DIFdelbegin \DIFdel{$s$)}\DIFdelend \DIFaddbegin \DIFadd{$S$), where each element of $\mathbf{\Phi}$ corresponds to a species' cue rate $\phi_s$}\DIFaddend .
We then have:

$$\log \mathbf{\Phi} \sim \DIFdelbegin \DIFdel{MVN}\DIFdelend \DIFaddbegin \DIFadd{N_{[S]}}\DIFaddend \left( \mathbf{\mu}\DIFdelbegin \DIFdel{_{[s]}}\DIFdelend \DIFaddbegin \DIFadd{_{[S]}}\DIFaddend , \mathbf{\Sigma}\DIFdelbegin \DIFdel{_{[s \times s]} }\DIFdelend \DIFaddbegin \DIFadd{_{[S \times S]} }\DIFaddend \right)\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend $$
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \par %%%
\DIFdelend \DIFaddbegin \DIFadd{where $N_{[S]}(\cdot,\cdot)$ denotes a multivariate normal distribution of length $S$.
}\DIFaddend The covariance function $\mathbf{\Sigma}$ (denoted with subscript \DIFdelbegin \DIFdel{$[s \times s]$ }\DIFdelend \DIFaddbegin \DIFadd{$[S \times S]$ }\DIFaddend to show dimensionality) determines the strength of the partial pooling, and can be any function that contains a measure of distance. 
In this case, we will create a function based on phylogenetic distance \DIFdelbegin \DIFdel{$\mathbf{C}_{[s \times s]}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{C}_{[S \times S]}$}\DIFaddend , i.e.,

$$\mathbf{\Sigma}\DIFdelbegin \DIFdel{_{[s \times s]} }\DIFdelend \DIFaddbegin \DIFadd{_{[S \times S]} }\DIFaddend = \mathbf{C}\DIFdelbegin \DIFdel{_{[s \times s]}}\DIFdelend \DIFaddbegin \DIFadd{_{[S \times S]}}\DIFaddend \lambda\sigma.$$
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \par %%%
\DIFdelend Here, $\lambda$ is Pagel's Lambda \citep{pagel_inferring_1999}, which determines the strength of the phylogenetic relationship. 
We set $\lambda = 0.76$, consistent with previous results \cite{solymos_phylogeny_2018}. 
\DIFdelbegin \DIFdel{We could have chosen to draw estimates from a distribution of possible $\lambda$ as in \mbox{%DIFAUXCMD
\citet{solymos_phylogeny_2018}}\hskip0pt%DIFAUXCMD
, but we chose not to for simplicity}\DIFdelend \DIFaddbegin \DIFadd{The effect of this is to only scale the strength of the phylogenetic relationship; had we not set a value for this, the scaling strength would have been captured in the estimate of $\sigma$. 
However, we chose to be explicit here consistent with previous results of \mbox{%DIFAUXCMD
\cite{solymos_phylogeny_2018}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
%DIFDELCMD < \par %%%
\DIFdelend Finally, the vector of mean cue rate \DIFdelbegin \DIFdel{$\mathbf{\mu}_{[s]}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\mu}_{[S]}$ }\DIFaddend is a function of the species migratory strategy $\alpha_M$, where the migratory strategy $M$ could either be resident or migrant. 
...
\subsubsection{Bayesian Hierarchical QPAD Distance Model}\DIFaddbegin \label{section-distance-models}
\DIFaddend 

\par \DIFdelbegin \DIFdel{For distance modelling, we are only interested in the distance to the cueing bird, and not the time bin that the bird was recorded in, and so we sum counts from each sampling event $i$ over all time bands. 
Thus, we consider counts $Y_{si.k} = \sum_{j=1}^{J}$. 
From \mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
, we model $Y_{si.k}$ as
}\begin{displaymath}\DIFdel{Y_{si.k} \sim multinomial\left(Y_{si..}, \mathbf{\Pi}_{si.}\right).}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \par \DIFdelbegin \DIFdel{For sample $i$, $\mathbf{\Pi}_{si.}$ is the corresponding probability vector that determines how the total number of species $s$ observed at sample $i$ (i.e., $Y_{si..}$) are to be distributed across the $K$ distance bins; in essence, it is the mixing parameter . 
Let $r_{ik}$ be the maximum distance for distance bin $k$ during sampling event $i$, and let $\tau_s$ be the unknown effective detection radius for species $s$.
...
%DIFDELCMD < %%%
\DIFdel{where 
}\begin{displaymath}\DIFdel{f(r,\tau) =  1 - \exp\left\{ -\dfrac{r^2}{\exp\left\{\tau^2\right\}} \right\} .}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < \par %%%
\DIFdel{We are most interested in estimating $\tau_s$, the effective detection radius for species $s$. 
}\DIFdelend \DIFaddbegin \DIFadd{Recall from Equation \ref{eq:distance} that the EDR $\tau$ determines the mixing parameter in a distance model setting.
}\DIFaddend To allow for the partial pooling of estimates for each species to be based on traits, we set up a mixed effects model as follows:
...
That is, we have
$$ \mu_s = \alpha_0 + \alpha_{M_s} + \alpha_{H_s} + \beta_b \times \log BodySize_s + \beta_p \times \log SongPitch_s\DIFaddbegin \DIFadd{.}\DIFaddend $$
\DIFaddbegin 

\DIFadd{We then set the following priors, based on previous results from \mbox{%DIFAUXCMD
\citet{solymos_phylogeny_2018}}\hskip0pt%DIFAUXCMD
:
}\DIFaddend $$\alpha_0 \sim N(0.05, 0.1)$$
$$ \alpha_M, \alpha_H \sim N(0, 0.05)$$
$$ \beta_b \sim N(0.01, 0.005)$$
$$ \beta_p \sim N(-0.01, 0.005) $$
$$\sigma \sim exp(5)$$

\subsection{\DIFdelbegin \DIFdel{Single Species }\DIFdelend \DIFaddbegin \DIFadd{Single-species }\DIFaddend Models}
...
The \DIFdelbegin \DIFdel{single species }\DIFdelend \DIFaddbegin \DIFadd{single-species }\DIFaddend models used the same likelihood statements as \DIFdelbegin \DIFdel{above }\DIFdelend \DIFaddbegin \DIFadd{Equations \ref{eq:removal} and \ref{eq:distance} }\DIFaddend for the count data, except rather than cue rate and EDR being estimated with shared information, they are estimated independently from each other.
That is, \DIFdelbegin \DIFdel{$\log \phi_s \sim N(0,1) \forall s$, and $\log \tau_s \sim N(0,1) \forall s$, thus making it equivalent to the Null model in \mbox{%DIFAUXCMD
\citet{edwards_point_2023}}\hskip0pt%DIFAUXCMD
, but estimated in a Bayesian rather than maximum likelihood framework}\DIFdelend \DIFaddbegin \DIFadd{$\log \phi_s \sim N(0,1)$, and $\log \tau_s \sim N(0,1)$, implying that the values of $\phi$ and $\tau$ are drawn as independent samples}\DIFaddend .
...
\DIFdelbegin \DIFdel{We did not include Woodhouse's Scrub-jay (}\textit{\DIFdel{Aphelocoma woodhouseii}}%DIFAUXCMD
\DIFdel{), Pacific Wren (}\textit{\DIFdel{Troglodytes pacificus}}%DIFAUXCMD
\DIFdel{), or Sagebrush Sparrow (}\textit{\DIFdel{Artemisiospiza nevadensis}}%DIFAUXCMD
\DIFdel{) in the analysis, because these species were not available in the phylogenies from \mbox{%DIFAUXCMD
\citet{jetz_global_2012}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend Rufous Hummingbird (\textit{Selasphorus rufus}) was dropped from the distance modelling due to issues with divergent transitions \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{betancourt_diagnosing_2016, leimkuhler_simulating_2005}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\citet{betancourt_diagnosing_2016, leimkuhler_simulating_2005}}\hskip0pt%DIFAUXCMD
; see Discussion)}\DIFaddend .
Tricolored Blackbird was only included in the distance model because there \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were only }\DIFaddend sufficient distance sampling data.
This resulted in a total of 316 species for removal modelling, and 315 species for distance modelling.
We also ran the \DIFdelbegin \DIFdel{single species }\DIFdelend \DIFaddbegin \DIFadd{single-species }\DIFaddend removal and distance models for the same suite of birds.
We then modelled the \DIFdelbegin \DIFdel{multi species }\DIFdelend \DIFaddbegin \DIFadd{multi-species }\DIFaddend estimates of cue rate and EDR as functions of the \DIFdelbegin \DIFdel{single species }\DIFdelend \DIFaddbegin \DIFadd{single-species }\DIFaddend estimates of cue rate and EDR, and obtained estimates of slope and y-intercept for this relationship.
If our prediction is true, we would expect to see a slope of 1 and a y-intercept of 0, indicating a 1-to-1 relationship.

\par To test our prediction about improved precision, we plotted the species-specific \DIFdelbegin \DIFdel{estimates of }\DIFdelend \DIFaddbegin \DIFadd{posterior }\DIFaddend standard deviations of cue rate and EDR, respectively, against the log sample size, for each of the single-species and multi-species models.
...
That is, for each of the five cross-validation runs, we held out 20\% of the data for each species for both the multi-species and single-species models \DIFaddbegin \DIFadd{and ran the models with the remaining 80\% of the data}\DIFaddend . 
We then derived multi-species estimates of cue rate and EDR and single-species estimates of cue rate and EDR, for each cross-validation run, and used these parameter estimates to calculate the estimated log pointwise posterior density (LPPD\DIFaddbegin \DIFadd{; \mbox{%DIFAUXCMD
\citet{gelman_understanding_2013}}\hskip0pt%DIFAUXCMD
}\DIFaddend ) of each of the held out data points\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{. 
The LPPD of each of the held out data points }\DIFaddend tells us the likelihood of the data point under the estimated parameters.
\DIFaddbegin \DIFadd{Higher values of LPPD indicate a higher likelihood of that data point under the estimated parameters, and so a higher LPPD value of a held-out data point implies that the model has a good predictive capacity for that data point.
}\DIFaddend We then calculated the difference in LPPD for removal and distance models, by subtracting the pointwise LPPD of the single-species model from the LPPD of the multi-species model (i.e., multi-species minus single-species).
...
If our prediction is true, then we would expect to see \DIFdelbegin \DIFdel{majority }\DIFdelend \DIFaddbegin \DIFadd{predominantly }\DIFaddend positive values of the species-specific mean LPPD for species with lower sample sizes.
...
\par \DIFaddend To determine which species would be left out in each fold of the removal model \DIFaddbegin \DIFadd{cross validation}\DIFaddend , we first split the phylogenetic tree into 10 parts using the \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend cutree" function in base R, which preserves relatedness in the 10 parts of the split tree.
Then, for each of the parts of the tree, we randomly assigned a number between 1 and 10 (drawn from a uniform distribution) \DIFdelbegin \DIFdel{to all migratory }\DIFdelend \DIFaddbegin \DIFadd{separately for all migratory and }\DIFaddend then all resident species.
...
\par \DIFaddend To determine which species would be left out in each fold of the distance model \DIFaddbegin \DIFadd{cross validation}\DIFaddend , we randomly assigned a number between 1 and 10 (drawn from a uniform distribution) to each species within a migratory \DIFdelbegin \DIFdel{resident }\DIFdelend \DIFaddbegin \DIFadd{status }\DIFaddend $\times$ habitat preference \DIFaddbegin \DIFadd{grouping combination}\DIFaddend , which ensured that there would be a good balance of species left out for each group of species.
\DIFdelbegin \DIFdel{Because Stan allows draws from a posterior distribution even with no data}\DIFdelend \DIFaddbegin 

\DIFadd{For each cross-validation fold}\DIFaddend , we drew values from the posterior distribution of \DIFdelbegin \DIFdel{all the held out speciesfor each cross-validation fold}\DIFdelend \DIFaddbegin \DIFadd{the held-out species}\DIFaddend , and used mean and standard deviation of those draws as the predicted cue rate or EDR.
...
\par \DIFaddend For two of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these seven }\DIFaddend species, Kirtland's Warbler and Bicknell's Thrush, we used the predicted cue rate and EDR to calculate availability and perceptibility \DIFaddbegin \DIFadd{(using Equations \ref{eq:availability} and \ref{eq:perceptibility})}\DIFaddend , and hence detectability, for a given survey type, and compared these metrics to previous estimates of detectability obtained from other studies.
\DIFdelbegin \DIFdel{As a reminder, following derivations in \mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
, detectability is the product of availability and perceptibility.
...
%DIFDELCMD < %%%
\begin{displaymath}
	\DIFdel{p(t) = 1 - e^{-t\phi}
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $\phi$ is the cue rate of the species.
Perceptibility $q(r)$ is defined as the probability of an observer hearing or seeing a bird, given the bird gives a cue, with a survey radius of $r$ metres, and is calculated as
	...
	\DIFdel{q(r) = \dfrac{\tau^2 \left\{1 - e^{-\dfrac{r^2}{\tau^2}}\right\}}{r^2}
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{where $\tau$ is the EDR of the species.
}\DIFdelend For Kirtland's Warbler, we compared our estimates of detectability to those obtained in \citet{van_dyke_comparative_2022} which compared estimates of Kirtland's Warbler detectability among different jack pine stands and red pine stands.
...
All Bayesian models were coded in Stan \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{stan_development_team_stan_2024}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{stan_development_team_stan_2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and models were run using the \texttt{cmdstanr} R package \citep{gabry_cmdstanr_2023} and visualized using the \texttt{bayesplot} R package \citep{gabry_visualization_2019}.
...
\par The k-fold cross validation revealed that the multi-species removal model had better predictive performance for more species (200 out of 316) than the single-species removal model (94 out of 316)\DIFaddbegin \DIFadd{, as determined by differences in species-wise mean LPPD}\DIFaddend .
...
Thus, it is possible that data for these species are \DIFdelbegin \DIFdel{biased high}\DIFdelend \DIFaddbegin \DIFadd{higher than what might be expected based on traits alone}\DIFaddend , because an observer would be able to identify such a species even at extreme distances.
...
Therefore, while the multi-species implementation can somewhat improve precision of estimates for lower sample size, the use of even \DIFdelbegin \DIFdel{weakly-informed }\DIFdelend \DIFaddbegin \DIFadd{weakly-informative }\DIFaddend priors clearly goes a long way in improving precision.
We therefore recommend future implementations of the NA-POPS detectability database to adopt a Bayesian modelling approach using at least \DIFdelbegin \DIFdel{weakly-informed }\DIFdelend \DIFaddbegin \DIFadd{weakly-informative }\DIFaddend priors.
...
One potential mechanism behind this is how birds are recorded in removal sampling, in that birds are recorded in the first time bin that they are observed, and then \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend removed" from the available birds to sample from for the remainder of the survey.
In grassland environments, where visual sightings are potentially more likely (and combined with less sound attenuation from vegetation), it is likely that the majority of these birds are being recorded in the first time bin, \DIFdelbegin \DIFdel{potentially inflating the }\DIFdelend \DIFaddbegin \DIFadd{which would cause a higher (though not necessarily incorrect) }\DIFaddend cue rate when \DIFdelbegin \DIFdel{data is available}\DIFdelend \DIFaddbegin \DIFadd{this data is used}\DIFaddend .
Then, when the species is modelled without the data, the cue rate is shrunk toward \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend similar" species that may occur in both open and closed environments, therefore shrinking the cue rate down to a lower value.
...
\par Given the results of the k-fold cross-validation, we were comfortable with applying this model to species where we had \DIFdelbegin \DIFdel{only a few data points }\DIFdelend \DIFaddbegin \DIFadd{fewer data points than what one would typically use for removal and distance modelling}\DIFaddend , such as Lesser Prairie-chicken, Spotted Owl, LeConte's Thrasher, Harris's Sparrow, and Tricolored Blackbird.
...
\DIFdelbegin \DIFdel{We note that even though we used a Gaussian process to estimate cue rate, we did not have to estimate the covariance matrix used for the Gaussian process, because it was effectively a known parameter provided by the phylogenetic relationship.
This would account for the relatively quick performance of this model given the number of parameters to estimate and the amount of data used; had the covariance matrix needed to be estimated, this model would have likely taken much longer to complete}\DIFdelend \DIFaddbegin \DIFadd{For distance modelling, after accounting for the issues previously mentioned, all models finished within approximately 36 hours, and all parameters had R-hat values of 1, or within 0.001 of it}\DIFaddend .
...
With this in mind, it may be beneficial to include temporal covariates at the survey level, following \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013, solymos_evaluating_2018, edwards_point_2023}}\hskip0pt%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{solymos_evaluating_2018}}\hskip0pt%DIFAUXCMD
, and \mbox{%DIFAUXCMD
\citet{edwards_point_2023}}\hskip0pt%DIFAUXCMD
, }\DIFaddend so that further information about cue rate may be shared among similar species giving cues at similar times of day or year.
...
\DIFaddbegin \DIFadd{This was beyond the scope of our study because we were only interested in assessing the use of a multi-species model as it pertained to phylogeny and traits, and not to these temporal and environmental covariates.
}\DIFaddend 
...
\begin{figure}[h]
	...
	\caption{1-to-1 plot comparing cue rates (A) and EDRs (B) estimated from the single-species model vs the multi-species model for each species. Red dashed line denotes the 1-to-1 line. \DIFdelbeginFL \DIFdelFL{Black line is regression line with posterior draws in grey. }\DIFdelendFL Grey dots are individual species.}
	\label{fig:1vs1}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{\DIFdelbeginFL \DIFdelFL{Group parameter posterior }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Posterior group-level hyperparameter }\DIFaddendFL distributions for multi-species removal model (A) and multi-species distance model (B). \DIFaddbeginFL \DIFaddFL{Circles are median values with thick bars being 50\% credible interval and thin bars being 90\% credible intervals.}\DIFaddendFL }
	\label{fig:params}
\end{figure}

\begin{figure}[h]
	\includegraphics{../output/plots/kfold_cv_plot.png}
	\caption{Mean difference in \DIFaddbeginFL \DIFaddFL{log }\DIFaddendFL pointwise \DIFdelbeginFL \DIFdelFL{posterior }\DIFdelendFL predictive density \DIFaddbeginFL \DIFaddFL{(LPPD) }\DIFaddendFL for cue rate (A) and \DIFdelbeginFL \DIFdelFL{EDR }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{effective detection radius }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{EDR; }\DIFaddendFL B) vs. log species sample size. Points are mean difference by species. Difference was calculated as multi-species minus single-species, and so positive differences (coloured in red for clarity) represent a multi-species model preference, whereas negative differences (coloured in blue for clarity) represent a single-species model preference. Grey points denote no differences. }
	\label{fig:cv}
\end{figure}

\begin{figure}[h]
	\includegraphics{../output/plots/species_cv_plot.png}
	\caption{(A) Estimates of cue rate for a species (black dots) given full data versus predictions of cue rate for the same species given no data. (B) Estimates of effective detection radius (EDR) for a species (black dots) given full data versus predictions of EDR for the same species given no data. For both, red dashed line denotes the 1-to-1 line.\DIFdelbeginFL \DIFdelFL{Black line is regression line with posterior draws in grey.}\DIFdelendFL }
	\label{fig:species_cv}
\end{figure}

\begin{figure}[h]
	\includegraphics{../output/plots/species_cv_distance_traits.png}
	\caption{Estimates of effective detection radius (EDR) for a species (dots) given full data versus predictions of EDR for the same species given no data, focusing in on species with EDRs between 60 m and 150 m. Colours denote discrete trait group combinations of migratory strategy and habitat preference. Red dashed line denotes the 1-to-1 line\DIFdelbeginFL \DIFdelFL{. Black line is regression line with posterior draws in grey}\DIFdelendFL .}
